...
- 本文档不包含具体数据结构、字段名、默认值、函数签名等；这些应在后续 OpenSpec change 的 `proposal / design` 中定义与收敛。
...
- `P3(v1)` 的传输选项为：`kcp`、`quic`、`brutal`。
- `brutal` 是一个传输名称，表示“基于 `QUIC` 的 `brutal` 调度策略”；它不是完整 `Hysteria2` 产品协议兼容层。
...
- 数据面必须支持显式选择 `kcp | quic | brutal`，并允许携带该传输所需的最小参数集合（具体 schema 在后续 OpenSpec change 中定义）。
- `P3(v1)` 优先保持最小可用模型（单会话、单传输、单流），不预设复杂插件系统，也不提前承诺“完整多流框架”。
- `P3(v1)` 允许不同传输在内部维持不同实现细节，但外层端侧流程必须通过统一入口接入，避免再把 `kcp / quic / brutal` 写成散落分支。

## 消息交换约束

- `P3(v1)` 不单独增加新的传输协商往返；传输选择复用现有 `exchange` 消息流。
- `visitor` 与 `client` 两端都必须显式声明本次会话的传输选择（含必要参数）；协调端负责校验并回传最终选定结果。
...
- 参数的具体 schema、默认值与边界规则在后续 OpenSpec change 的 `proposal / design` 中定义；本文档不锁定这些细节。
...
- 单元测试至少覆盖：
  ...
  - 传输层观测事件完整性
- 集成回归至少覆盖：
  - `core-01`：`data=kcp`、`data=quic`、`data=brutal` 均可成功建立并交换 payload
  - `core-01-loss`：至少验证 `data=brutal` 在代表性高丢包场景下仍可完成 payload 交换
