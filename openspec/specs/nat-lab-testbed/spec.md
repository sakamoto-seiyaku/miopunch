# nat-lab-testbed Specification

## Purpose
`nat-lab-testbed` 定义并约束 `P0` 阶段的 NAT 仿真与回归环境：以单个 `QEMU VM` 作为隔离实验母机，在 VM 内用 `netns/veth/iptables/tc` 构建可切换的 NAT 拓扑，并通过可验证的 profile 与可复盘 artifacts（日志/抓包/规则/conntrack/tc）保证实验结果“可复现、可回归、可排障”，且不对默认 `Windows/WSL2` 网络做持久改动。
## Requirements
