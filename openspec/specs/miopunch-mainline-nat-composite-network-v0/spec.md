...
The default public MNT-03 gates SHALL validate those checkpoints as one continuously growing network in the QEMU lab VM with real Docker/systemd `miopunch` nodes and lab-controlled NAT fixtures. A later checkpoint MUST NOT be treated as complete until the same progressive run has produced passing machine-readable evidence for the earlier checkpoints.
