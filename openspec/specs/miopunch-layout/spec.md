# miopunch-layout Specification

## Purpose
`miopunch-layout` defines the repository package layout and naming convergence goals for `P3`.
It removes the top-level `xtcp/` namespace, establishes stable domain-oriented packages, and keeps the project buildable/testable while aligning actively maintained docs with current OpenSpec change locations.
