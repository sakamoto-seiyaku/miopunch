...
- **WHEN** a peer sends a NAT-hole request that includes `tcp_direct_addrs`, `tcp_mapped_addrs`, and `tcp_stun_cn/global`
- **THEN** the receiving side can decode the message and observe the same field values

### Requirement: Exchange response returns peer_tcp_direct_addrs and tcp_candidate_addrs
