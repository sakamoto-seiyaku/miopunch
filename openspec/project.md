...
- `Container / virtualization`：优先 `Docker`，必要时补充 `QEMU` 做系统级网络实验。
...
- 集成测试覆盖：基于容器或虚拟化的 NAT 拓扑、链路质量扰动、回归基线。
- 真实环境验证覆盖：安卓蜂窝、本地宽带、云服务器等真实网络组合。
...
- 优先做 `Linux-first` 的可复现测试台，再逐步扩展真实设备与跨平台验证。
