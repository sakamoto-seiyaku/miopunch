...
func (m *Manager) runInviteTask(taskID string, rawArgs []byte) {
	...
	inviteBroker := normalizeBrokerEndpoint(st.Local.MQTTBroker)
	if strings.TrimSpace(inviteBroker) == "" {
		m.addFact(taskID, poc.Fact{Message: "missing mqtt_broker in local state"})
		m.addSuggestion(taskID, poc.Suggestion{Message: "set local.mqtt_broker in state.json and retry"})
		m.done(taskID, poc.ReasonCodeBadRequest, poc.ExitCodeBadRequest)
		return
	}
