...
// NOTE: Derived from Hysteria2's Brutal sender implementation.
// Source: apernet/hysteria (tag: app/v2.7.1), core/internal/congestion/brutal/brutal.go
...
func (b *BrutalSender) updateAckRate(currentTimestamp int64) {
	...
	if ackCount+lossCount < minSampleCount {
		...
		if b.canPrintAckRate(currentTimestamp) {
			...
			b.debugPrint(
				"Not enough samples (total=%d, ack=%d, loss=%d, rtt=%d)",
				ackCount+lossCount,
				ackCount,
				lossCount,
				b.rttStats.SmoothedRTT().Milliseconds(),
			)
		}
		return
	}
	...
	if b.canPrintAckRate(currentTimestamp) {
		b.lastAckPrintTimestamp = currentTimestamp
		b.debugPrint(
			"Ack rate updated (total=%d, ack=%d, loss=%d, ackRate=%.2f, rtt=%d)",
			ackCount+lossCount,
			ackCount,
			lossCount,
			b.ackRate,
...
func (b *BrutalSender) canPrintAckRate(currentTimestamp int64) bool {
	...
	return currentTimestamp-b.lastAckPrintTimestamp >= debugPrintInterval
}

func (b *BrutalSender) debugPrint(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "[brutal] "+format+"\\n", args...)
}
