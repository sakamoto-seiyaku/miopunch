...
// Source: apernet/hysteria (tag: app/v2.7.1), core/internal/congestion/bbr/windowed_filter.go
//
// The original code used golang.org/x/exp/constraints. We keep miopunch's
// dependency surface smaller by defining the minimal type sets locally.

// Implements Kathleen Nichols' algorithm for tracking the minimum (or maximum)
// estimate of a stream of samples over some fixed time interval. (E.g. the
// minimum RTT over the past five minutes.) The algorithm keeps track of the
// best, second best, and third best min (or max) estimates, maintaining an
...
// MaxFilter compares two values.
func MaxFilter[O ordered](a, b O) int {
	if a > b {
		return 1
	} else if a < b {
		return -1
...
// MinFilter compares two values.
func MinFilter[O ordered](a, b O) int {
	if a < b {
		return 1
	} else if a > b {
		return -1
...
// SetWindowLength changes the window length. It does not update any current samples.
...
func (f *WindowedFilter[V, T]) Update(newSample V, newTime T) {
	// Reset all estimates if they have not yet been initialized, if new sample is a new best,
	...
	if newTime-f.estimates[0].time > f.windowLength {
		// The best estimate hasn't been updated for an entire window, so promote second and third best estimates.
		f.estimates[0] = f.estimates[1]
		f.estimates[1] = f.estimates[2]
		f.estimates[2] = entry[V, T]{newSample, newTime}
		// Need to iterate one more time. Check if the new best estimate is outside the window as well,
		// since it may also have been recorded a long time ago. Don't need to iterate once more since
		// we cover that case at the beginning of the method.
		if newTime-f.estimates[0].time > f.windowLength {
			f.estimates[0] = f.estimates[1]
			f.estimates[1] = f.estimates[2]
		}
	...
	if f.comparator(f.estimates[1].sample, f.estimates[0].sample) == 0 &&
		...
		// A quarter of the window has passed without a better sample, so the second-best estimate is taken
		// from the second quarter of the window.
		f.estimates[1] = entry[V, T]{newSample, newTime}
		f.estimates[2] = f.estimates[1]
		return
	}
	...
	if f.comparator(f.estimates[2].sample, f.estimates[1].sample) == 0 &&
		...
		// We've passed a half of the window without a better estimate, so take a third-best estimate
		// from the second half of the window.
		f.estimates[2] = entry[V, T]{newSample, newTime}
	}
}
