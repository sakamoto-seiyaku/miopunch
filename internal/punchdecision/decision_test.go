...
func NewController(dataReserveDuration time.Duration) (*testController, error) {
	return &testController{engine: NewEngine(dataReserveDuration)}, nil
}

func (c *testController) analysis(session *Session) (*wire.NatHoleResp, *wire.NatHoleResp, error) {
	result, err := c.engine.Analyze(session.sid, session.visitorMsg, session.clientMsg)
	if err != nil {
		return nil, nil, err
	}
	session.vNatFeature = result.VisitorNATFeature
	session.cNatFeature = result.ClientNATFeature
